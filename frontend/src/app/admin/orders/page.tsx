...
import { 
  ...
  Package, MapPin, Truck, Save, Phone 
} from 'lucide-react';
...
export default function AdminOrders() {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    try {
      ...
      // Update locally, preserve populated products
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-6 space-y-4 md:space-y-0 gap-4">
          ...
          <button 
            ...
            className={`flex items-center justify-center space-x-2 px-6 py-3 rounded-lg border transition-colors font-montserrat text-sm tracking-widest uppercase cursor-pointer ${
              showFilters || statusFilter !== 'All' || paymentStatusFilter !== 'All' || paymentMethodFilter !== 'All'
                ? 'bg-[#ff0033]/10 border-[#ff0033]/30 text-[#ff0033] hover:bg-[#ff0033]/20'
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 p-5 bg-foreground/5 border border-border rounded-xl backdrop-blur-md font-poppins">
                {/* Order Status Filter */}
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Order Status</label>
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:border-[#ff0033] cursor-pointer"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Payment Status</label>
                  <select
                ...
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Payment Method</label>
                  <select
                ...
                {/* Sorting */}
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Sort By</label>
                  <select
                    value={sortBy}
        ...
        <div className="md:hidden space-y-4">
          {loading ? (
            ...
            filteredOrders.map((order) => (
              <div key={order._id} className="bg-foreground/5 border border-border rounded-xl p-4 flex flex-col space-y-3 shadow-sm dark:shadow-none font-poppins">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm font-bold text-foreground">#{order._id.substring(0, 10).toUpperCase()}</p>
                    <p className="text-xs text-foreground/50 mt-0.5">{new Date(order.createdAt).toLocaleDateString()}</p>
                  </div>
                ...
                <div className="pt-2 border-t border-border/50 flex flex-col space-y-2 text-xs">
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-foreground/90 font-medium">{order.userId?.name || 'Guest'}</p>
                      <p className="text-foreground/50">{order.userId?.email || 'N/A'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-foreground/50 font-montserrat uppercase tracking-wider text-[10px]">Total Amount</p>
                      <p className="text-foreground font-bold text-sm">₹{order.totalAmount}</p>
                    </div>
                  </div>
                ...
                {expandedOrderId === order._id && (
                  <div className="pt-4 border-t border-border/30 space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        {order.products?.map((item: any, idx: number) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center justify-between p-2.5 bg-background/50 border border-border/50 rounded-lg">
                              <div className="flex items-center space-x-2.5">
                                <div className="w-10 h-13 bg-foreground/5 rounded overflow-hidden flex-shrink-0 border border-border/50">
                                  {displayImg ? (
                                    <img src={displayImg} alt="" className="w-full h-full object-cover" />
                                  ) : (
                                    <div className="w-full h-full flex items-center justify-center text-[8px] text-foreground/30 font-bold font-montserrat">N/A</div>
                                  )}
                                </div>
                                <div>
                                  <p className="text-xs font-semibold text-foreground max-w-[150px] truncate">{displayTitle}</p>
                              ...
                              <div className="text-right">
                                ...
                                <p className="text-[9px] text-foreground/50">Qty: {item.quantity}</p>
                              </div>
                            </div>
                          );
                        })}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="p-3 bg-background/50 border border-border/50 rounded-lg space-y-1.5 text-[11px]">
                        <p className="font-semibold text-foreground">{order.shippingAddress?.name || order.userId?.name}</p>
                        <p className="text-foreground/85 leading-relaxed">
                          {order.shippingAddress?.street}, {order.shippingAddress?.city}, {order.shippingAddress?.state} - {order.shippingAddress?.zipCode}, {order.shippingAddress?.country}
                        </p>
                        {order.shippingAddress?.phone && (
                          <p className="text-foreground/70 flex items-center gap-1.5 pt-1.5 border-t border-border/25 text-[10px]">
                            <Phone size={10} className="text-foreground/50" />
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="p-3 bg-background/50 border border-border/50 rounded-lg space-y-3">
                        <div className="space-y-1.5">
                          <label className="block text-[8px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Courier Partner</label>
                          <input 
                            type="text" 
                            placeholder="e.g. Delhivery, Bluedart" 
                            value={editingCourier}
                            onChange={(e) => setEditingCourier(e.target.value)}
                            className="w-full bg-background border border-border rounded-lg px-2.5 py-1.5 text-xs text-foreground outline-none focus:border-[#ff0033]"
                          />
                        </div>
                        <div className="space-y-1.5">
                          <label className="block text-[8px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Tracking ID</label>
                          <input 
                            type="text" 
          ...
          <div className="overflow-x-auto min-h-[400px]">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border">
                {loading ? (
                  <tr><td colSpan={7} className="text-center py-12 text-foreground/50 font-poppins">Loading Orders...</td></tr>
                ) : filteredOrders.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-12 text-foreground/50 font-poppins">No orders found.</td></tr>
                ) : (
                  filteredOrders.map((order) => (
                    <React.Fragment key={order._id}>
                      <tr className="hover:bg-foreground/[0.02] transition-colors border-b border-border/40 font-poppins">
                        <td className="px-4 py-4 text-center">
                          <button 
                            onClick={() => toggleExpandOrder(order)}
                            className="p-1 rounded hover:bg-foreground/10 text-foreground/70 transition-colors cursor-pointer bg-transparent border-none outline-none"
                          >
                            {expandedOrderId === order._id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                          </button>
                        </td>
                        <td 
                          className="px-6 py-4 text-sm font-medium text-foreground cursor-pointer"
                          onClick={() => toggleExpandOrder(order)}
                        >
                        ...
                        <td 
                          ...
                          onClick={() => toggleExpandOrder(order)}
                        >
                          <p className="text-sm text-foreground/90 font-medium">{order.userId?.name || 'Guest'}</p>
                          <p className="text-xs text-foreground/50">{order.userId?.email || 'N/A'}</p>
                        </td>
                        <td 
                          className="px-6 py-4 text-sm text-foreground/70 cursor-pointer"
                          onClick={() => toggleExpandOrder(order)}
                        >
                      ...
                      {expandedOrderId === order._id && (
                        <tr className="bg-foreground/[0.01]">
                          <td colSpan={7} className="px-8 py-6 border-b border-border/60">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-foreground">
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                  {order.products?.map((item: any, idx: number) => {
                                    ...
                                    const displayImg = prod?.images?.[0] ? (typeof prod.images[0] === 'string' ? prod.images[0] : prod.images[0].url) : '';
                                    return (
                                      <div key={idx} className="flex items-center justify-between p-3 bg-background/50 border border-border/50 rounded-xl">
                                        <div className="flex items-center space-x-3">
                                          <div className="w-12 h-15 bg-foreground/5 rounded overflow-hidden flex-shrink-0 border border-border/50">
                                            {displayImg ? (
                                              <img src={displayImg} alt="" className="w-full h-full object-cover" />
                                            ) : (
                                              <div className="w-full h-full flex items-center justify-center text-[10px] text-foreground/30 font-bold font-montserrat">N/A</div>
                                            )}
                                          </div>
                                          <div>
                                            <p className="text-xs font-semibold text-foreground max-w-xs truncate">{displayTitle}</p>
                                            <p className="text-[10px] text-foreground/50 mt-0.5">
                                              Size: <span className="font-bold text-foreground/85">{item.size || 'N/A'}</span> | Color: <span className="font-bold text-foreground/85">{item.color || 'N/A'}</span>
                                            </p>
                                          </div>
                                        </div>
                                        <div className="text-right">
                              ...
                              <div className="space-y-6">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="p-4 bg-background/50 border border-border/50 rounded-xl space-y-2 text-xs">
                                    <p className="font-semibold text-foreground">{order.shippingAddress?.name || order.userId?.name}</p>
                                    <p className="text-foreground/85 leading-relaxed">
                                      {order.shippingAddress?.street}, {order.shippingAddress?.city}, {order.shippingAddress?.state} - {order.shippingAddress?.zipCode}, {order.shippingAddress?.country}
                                    </p>
                                    {order.shippingAddress?.phone && (
                                      <p className="text-foreground/70 flex items-center gap-1.5 pt-1.5 border-t border-border/25">
                                        <Phone size={12} className="text-foreground/50" />
                                        <span>{order.shippingAddress.phone}</span>
                                      </p>
                                    )}
                                  </div>
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="p-4 bg-background/50 border border-border/50 rounded-xl space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                      <div className="space-y-1">
                                        <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Courier Partner</label>
                                        <input 
                                          type="text" 
                                          placeholder="e.g. Delhivery, Bluedart" 
                                          value={editingCourier}
                                          onChange={(e) => setEditingCourier(e.target.value)}
                                          className="w-full bg-background border border-border rounded-lg px-3 py-1.5 text-xs text-foreground outline-none focus:border-[#ff0033]"
                                        />
                                      </div>
                                      <div className="space-y-1">
                                        <label className="block text-[10px] font-montserrat uppercase tracking-wider text-foreground/50 font-bold">Tracking ID</label>
                                        <input 
                                          type="text" 
                                    ...
                                    <div className="flex justify-end pt-1">
                                      <button
                                        onClick={() => handleUpdateTracking(order._id)}
                                        disabled={updatingOrderId === order._id}
                                        className="bg-[#ff0033] hover:bg-[#cc0029] disabled:bg-zinc-700 text-white px-4 py-2 rounded-lg text-xs font-montserrat font-bold tracking-widest uppercase transition-all flex items-center gap-1.5 shadow-md shadow-[#ff0033]/15 active:scale-95 cursor-pointer border-none"
                                      >
                                        <Save size={12} />
                                        <span>{updatingOrderId === order._id ? 'Saving...' : 'Save Tracking'}</span>
                                      </button>
